...
namespace Proj7
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            employees[0].ChangeSalary(5);
            Console.WriteLine(employees[0].Salary);
#if false
            foreach(var emp in  employees) {
                Console.WriteLine($"{emp.Name} - {emp.MonthlySalary:F0}원");
            }
#else
            for (int i = 0; i < employees.Count; i++) {
                Console.WriteLine($"{employees[i].Name} - {employees[i].MonthlySalary:F1}원");
            }
#endif
        }
    }
